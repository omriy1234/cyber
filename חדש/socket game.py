...
def pysocket(): #פונקציה האחראית על מסך הpygame
    ...
    font = pygame.font.SysFont('Lucida Handwriting', 20) #סוגי הפונטים והגדלים השונים
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                str2 = ""
                strtime2 = ""
                if len(lnow) != 0 and lnow[-1] == '': #מוחק קטעים רקים מהקטע של השרת
                    del lnow[-1]
                if len(lnow) != 0 and len(lnow) != len(ltimenow): #מוסיף מעברים עם חסר(בגלל ההוספות הידניות)
                ...
                if button_1l.collidepoint(mouse_pos):
                    pygame.draw.rect(screen, [0, 255, 0], button_1l)  # draw button
                    ...
                    if(len(lmusic)>=1):
                        if nowb == "l1" and note.bool == False:
                            ...
                            note = Music(lmusic[0].split("stop")[0],lmusic[0].split("stop")[1])
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_1a)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    if(len(lmusic)>=1 and note.bool == True): #אם יש ערך שהתקבל
                        lnow.append(lmusic[0].split("stop")[0]) #הוסף אותו לשרת
                        ltimenow.append(lmusic[0].split("stop")[1])
                    else:
                        print("1a")
                elif button_2l.collidepoint(mouse_pos):
                    pygame.draw.rect(screen, [0, 255, 0], button_2l)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    if(len(lmusic)>=2):
                        if nowb == "l2" and note.bool == False:
                            ...
                            note = Music(lmusic[1].split("stop")[0],lmusic[0].split("stop")[1])
                            t2 = threading.Thread(target= note.play_music)
                            t2.start()
                            nowb = "l2"
                    else:
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_2a)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_3l)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    if(len(lmusic)>=3):
                        if nowb == "l3" and note.bool == False:
                            ...
                            note = Music(lmusic[2].split("stop")[0],lmusic[0].split("stop")[1])
                            t2 = threading.Thread(target= note.play_music)
                            t2.start()
                            nowb = "l3"
                    else:
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_3a)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_4l)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    if(len(lmusic)>=4):
                        if nowb == "l4" and note.bool == False:
                            ...
                            note = Music(lmusic[3].split("stop")[0],lmusic[0].split("stop")[1])
                            t2 = threading.Thread(target= note.play_music)
                            t2.start()
                            nowb = "l4"
                    else:
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_4a)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_5l)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    if(len(lmusic)>=5):
                        if nowb == "l5" and note.bool == False:
                            ...
                            note = Music(lmusic[4].split("stop")[0],lmusic[0].split("stop")[1])
                            t2 = threading.Thread(target= note.play_music)
                            t2.start()
                            nowb = "l5"
                    else:
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_5a)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_6l)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    if(len(lmusic)>=6):
                        if nowb == "l6" and note.bool == False:
                            ...
                            note = Music(lmusic[5].split("stop")[0],lmusic[0].split("stop")[1])
                            t2 = threading.Thread(target= note.play_music)
                            t2.start()
                            nowb = "l6"
                    else:
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_6a)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_7l)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    if(len(lmusic)>=7):
                        if nowb == "l7" and note.bool == False:
                            ...
                            note = Music(lmusic[6].split("stop")[0],lmusic[0].split("stop")[1])
                            t2 = threading.Thread(target= note.play_music)
                            t2.start()
                            nowb = "l7"
                    else:
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_7a)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_8l)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    if(len(lmusic)>=8 and note.bool == True):
                        if nowb == "l8" and note.bool == False:
                            ...
                            note = Music(lmusic[7].split("stop")[0],lmusic[0].split("stop")[1])
                            t2 = threading.Thread(target= note.play_music)
                            t2.start()
                            nowb = "l8"
                    else:
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_8a)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    if(len(lmusic)>=8):
                    ...
                    if nowb == "play" and note.bool == False: #אם נלחץ פעם נוספת
                        ...
                        pygame.draw.rect(screen, [0, 255, 0], button_melody)  # draw button
                        pygame.display.update()
                        clock.tick(fps)
                        note = Music(str(lnow).replace("[","").replace("]","").replace("'","").replace(",",""),str(ltimenow).replace("[","").replace("]","").replace("'","").replace(",",""))
                        t2 = threading.Thread(target= note.play_music)
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_revers)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    lnow2 = []
                    ltimenow2 = [0.0]
                    for i in lnow:
                        lnow2.append(str(i.split()[::-1]).replace("[","").replace("]","").replace("'","").replace(",","")) #הפיכת סדר התווים בתוך כל איבר ברשימה
                    for j in ltimenow:
                        ltimenow2.append(str(j.split()[::-1]).replace("[","").replace("]","").replace("'","").replace(",","")) #הפיכת סדר הזמנים בתוך כל איבר ברשימה
                    ...
                    root2.destroy()
                    print(filename)
                    if not(str(filename) == "" or str(filename)[::-1][0:4][::-1] != ".txt"): #בדיקה שהקובץ תקין
                        input_file = open(r''+ filename,'a') #פתיחת קובץ לעריכה
                        input_file.write((str(lnow).replace("[","").replace("]","").replace("'","").replace(",","").replace("  "," "))+ '\n' + str(ltimenow).replace("[","").replace("]","").replace("'","").replace(",","").replace("  "," "))
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_clear)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    lnow = [] #איפוס משתנים
                    ltimenow = []
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_delete_note)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    print (lnow)
                    if (len(lnow) > 0):
                        ...
                        ltimenow[-1] = str(ltimenow[-1]).replace("[","").replace("]","").replace("'","").replace(",","")
                        print (ltimenow)
                    ...
                    if str(enter_notes) != "None": #בדיקה אם התקבל ערך
                        ...
                        lnow.append(enter_notes.lower()) #הוספת המתקבל לרשימת הנגינה של השרצ
                    print ("enter")

                elif button_clear_sections.collidepoint(mouse_pos):
                    pygame.draw.rect(screen, [0, 255, 0], button_clear_sections)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    n = 0
                    while len(lmusic) != 0 and n < 8:#מחיקת 8 האיברים הראשונים ברשימת קטעים שהתקבלו מהלקוחות
                        n += 1
                        del lmusic[0]
                    print ("clear sections")

                elif button_random.collidepoint(mouse_pos):
                    pygame.draw.rect(screen, [0, 255, 0], button_random)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    lnow2 = []
                    ltimenow2 = []
                    if lnow != []:
                        nrandom=random.randint(0,len(lnow) -1) #בחירת מספר רנדומלי
                    while lnow != []:
                        lnow2.append(lnow.pop(nrandom)) #הכנסת האיבר עם האינדקס של הרנדום לרשימה חדשה
                    ...
                    pygame.draw.rect(screen, [0, 255, 0], button_delete_last_added)  # draw button
                    pygame.display.update()
                    clock.tick(fps)
                    if len(lnow) != 0:
                    ...
                    string = ""
                    strtime = ""
                    if not(str(filename) == "" or str(filename)[::-1][0:4][::-1] != ".txt"): #האם הקובץ מסוג תקין
                        input_file = open(r''+ filename,'r') #פתיחת הקובץ לעריכה
...
def socketpiano(): #פנקציה האחראית על הרשתות
    global lmusic
    global server_socket
    server_socket.bind(('192.168.1.16',1729))
    server_socket.listen(5)
    open_client_sockets = []
    while len(lmusic) == 0 or lmusic[0] != "False": #המשכת הרצת השרת
        rlist, wlist, xlist = select.select( [server_socket] + open_client_sockets, [], [] )
        for current_socket in rlist:
            if(len(lmusic) != 0 and lmusic[0] == "False"): #עצירת הלולאה אם התקבל לסיים
                break
                ...
                if data == "":
                    ...
                    if data.decode() != '':
                        lmusic.append(data.decode()) #הוספת הקלט מהלקוח לרשימה
                        print (lmusic)
...
def main():
    ...
    t1.start() #הרצת פונקציית האחראית על הרשתות
    pysocket() #במקביל לפונקציה האחראית על הpygame
